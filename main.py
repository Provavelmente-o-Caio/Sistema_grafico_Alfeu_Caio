...
class Canvas(QWidget):
    def __init__(self):
        ...
        # Window and step for navigation
        self.window = Window()
    ...
    def transform_coords(self, wx, wy):
        # Get viewport dimensions
        vp_width = self.width()
        vp_height = self.height()
        
        # Calculate scale factors
        sx = vp_width / self.window.width()
        sy = vp_height / self.window.height()
        
        # Use the smaller scale factor to prevent distortion
        s = min(sx, sy)
        
        # Center in viewport
        vp_x = (vp_width - self.window.width() * s) / 2
        vp_y = (vp_height - self.window.height() * s) / 2
        
        # Transform from window to viewport coordinates
        vx = vp_x + (wx - self.window.xmin) * s
        # Invert Y axis (window Y grows up, viewport Y grows down)
        vy = vp_height - (vp_y + (wy - self.window.ymin) * s)
...
class SideBar(QWidget):
    ...
    def add_object(self):
        ...
        if type_str == "Dot":
            ...
            if len(coords) < 3:
                self.console.log("Error: A polygon requires at least 3 coordinate pairs.")
                return
...
class MainWindow(QMainWindow):
    ...
    def add_example_wireframes(self):
        ...
        polygon = Wireframe("Square", ObjectType.POLYGON, 
                           [(-5, -5), (5, -5), (5, 5), (-5, 5)])
